...
class GDSync_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/gdsync-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/gdsync-public.js', array( 'jquery' ), $this->version, false );

	}

	function gdsync_receive_post() {
		$user_id = $this->gdsync_compare_keys();
		...
		if ( is_wp_error( $id ) ) {
			...
			$this->gdsync_segmented_post_hook( $id );
		}
		wp_die();
	}

	function gdsync_receive_image() {
		$user_id = $this->gdsync_compare_keys();
		wp_set_current_user( $user_id );
	...
	function gdsync_compare_keys() {
		global $wpdb;

		if ( ! isset( $_SERVER['HTTP_X_GDSYNC_SIGNATURE'] ) ) {
			wp_die( "{\"error\": \"HTTP header 'X-GDSync-Signature' is missing.\"}" );
		}
	...
	function gdsync_aggregate_post( $unique_identifier, $final_number, $real_title, $final_post ) {
		$final_content = '';
	...
	function gdsync_segmented_post_hook( $post_id ) {
		$post = get_post( $post_id );
		list( $final, $num, $unique_identifier, $real_title ) = explode( '::', $post->post_title );

		if ( ! empty( $real_title ) && ( $final == 'final' ) ) {
			$this->gdsync_aggregate_post( $unique_identifier, $num, $real_title, $post );
		} else {
